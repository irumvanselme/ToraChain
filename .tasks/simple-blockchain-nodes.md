...
3. When the vote is casted on the @apps/backend/, then it will be sent to the master node.
4. The master node can not process anything unless at least 3 nodes are connected.
5. It will send data to each node and ask for the hash. VALIDATE_BLOCK
6. If 2/3 nodes have the same hash, then it will send the block to all the nodes in the network with a task: NEW_BLOCK.
